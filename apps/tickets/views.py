...
def backlog_view(request, project_id):
    ...
    return render(request, 'tickets/backlog.html', {'project': project, 'issues': issues})
...
def issue_create_view(request, project_id):
    project = get_object_or_404(Project, id=project_id)  # Retrieve the project using the project ID
    if request.method == 'POST':
        form = IssueForm(request.POST)
        if form.is_valid():
            issue = form.save(commit=False)  # Create an issue instance but don't save to the database yet
            issue.project = project  # Associate the issue with the selected project
            issue.save()  # Now save the issue to the database
            return redirect('backlog_view', project_id=project.id)  # Redirect to the backlog view
    else:
        form = IssueForm(initial={'project': project})  # Pre-fill the form with the project

    return render(request, 'tickets/issue_create.html', {
        'form': form,
        'project': project,
    })
...
def update_issue_status(request, issue_id):
    if request.method == 'POST':
        try:
            ...
            new_status = data.get('new_status_id')  # Get the new status from AJAX request

            # Debug logging for incoming request
            print('Received data:', data)  # Log incoming data for debugging

            if new_status in dict(Issue.STATUS_CHOICES).keys():  # Validate against valid choices
                issue = Issue.objects.get(id=issue_id)
